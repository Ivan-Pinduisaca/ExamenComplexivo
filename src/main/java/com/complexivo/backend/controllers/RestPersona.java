...
import com.complexivo.backend.services.ServicioPersona;
...
public class RestPersona {
	...
	public ResponseEntity<List<Persona>> all() {
		...
		return ResponseEntity.ok(servicioPersona.findAll());
	}
	...
	public ResponseEntity<?> findId(@PathVariable Long id) {
		...
		try {
			persona = servicioPersona.findById(id);
	...
	public ResponseEntity<?> create(@Valid @RequestBody Persona persona, BindingResult result) {
		...
		try {
			personaNew = servicioPersona.save(persona);
		} catch (DataAccessException e) {
			/* Enviamos la respuesta, con un mensaje de error y un status 500 -> INTERNAL_SERVER_ERROR */
			response.put("Mensaje", "Error al guardar la persona en la Base de Datos");
			response.put("Error", e.getMessage().concat(": ").concat(e.getMostSpecificCause().getMessage()));
			return new ResponseEntity<Map<String, Object>>(response, HttpStatus.INTERNAL_SERVER_ERROR);
		}

		/* Enviamos la respuesta, con un mensaje de exito y un status 201 -> CREATED */
		response.put("Mensaje", "La persona se ha creado con exito!");
	...
	public ResponseEntity<?> update(@Valid @RequestBody Persona persona, BindingResult result, @PathVariable Long id) {
		...
		Persona personaActual = servicioPersona.findById(id);
		...
		try {
			...
			personaUpdate = servicioPersona.save(personaActual);
		} catch (DataAccessException e) {
			/* Enviamos la respuesta, con un mensaje de error y un status 500 -> INTERNAL_SERVER_ERROR */
		...
		/* Enviamos la respuesta, con un mensaje de exito y un status 201 -> CREATED */
		response.put("Mensaje", "La persona se actualizo con exito!");
