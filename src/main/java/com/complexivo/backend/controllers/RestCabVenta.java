...
import com.complexivo.backend.services.ServicioCabVenta;
...
public class RestCabVenta {
	...
	@Autowired
	private ServicioCabVenta servicioCabVenta;

	/* -------------------------- METODO PARA LLISTAR TODOS LOS REGISTROS -------------------------- */
	@GetMapping("/facturas")
	public ResponseEntity<List<CabVenta>> all() {
		/* Enviamos la respuesta con mensaje de exito y un status 200 -> OK */
		return ResponseEntity.ok(servicioCabVenta.findAll());
	}
	...
	public ResponseEntity<?> findId(@PathVariable Long id) {
		...
		if (cabVenta == null) {
			...
			response.put("mensaje",
					"La factura con ID: '".concat(id.toString()).concat("' no existe en la Base de Datos"));
		 ...
		 * 
		 *  */
		return ResponseEntity.ok(cabVenta);
	}

	/* -------------------------- METODO PARA INSERTAR UN REGISTRO -------------------------- */
	@PostMapping("/facturas")
	public ResponseEntity<?> create(@Valid @RequestBody CabVenta cabVenta, BindingResult result) {

		CabVenta cabVentaNew = null;
		Map<String, Object> response = new HashMap<>();
		...
		try {
			cabVentaNew = servicioCabVenta.save(cabVenta);
		} catch (DataAccessException e) {
		...
		response.put("mensaje", "La factura se ha creado con exito!");
		response.put("Factura", cabVentaNew);
		return new ResponseEntity<Map<String, Object>>(response, HttpStatus.CREATED);
	}

	/* -------------------------- METODO PARA ACTUALIZAR UN REGISTRO -------------------------- */
	@PutMapping("/facturas/{id}")
	public ResponseEntity<?> update(@Valid @RequestBody CabVenta cabVenta, BindingResult result,
			@PathVariable Long id) {

		CabVenta cabVentaActual = servicioCabVenta.findById(id);
		CabVenta cabVentaUpdate = null;
		Map<String, Object> response = new HashMap<>();
		...
		try {
			cabVentaActual.setCveFechaEmision(cabVenta.getCveFechaEmision());
			cabVentaActual.setCveNumFactura(cabVenta.getCveNumFactura());
			cabVentaActual.setCliente(cabVenta.getCliente());
			cabVentaActual.setTipoPago(cabVenta.getTipoPago());
			cabVentaActual.setDetVentas(cabVenta.getDetVentas());

			cabVentaUpdate = servicioCabVenta.save(cabVentaActual);
		} catch (DataAccessException e) {
			/* Enviamos la respuesta, con un mensaje de error y un status 500 -> INTERNAL_SERVER_ERROR */
		...
		/* Enviamos la respuesta, con un mensaje de exito y un status 201 -> CREATED */
		response.put("Mensaje", "La factura se actualizo con exito!");
		response.put("Factura", cabVentaUpdate);
		return new ResponseEntity<Map<String, Object>>(response, HttpStatus.CREATED);
	}
	...
	public ResponseEntity<?> delete(@PathVariable Long id) {
		...
		try {
			servicioCabVenta.deleteById(id);
		} catch (DataAccessException e) {
			/* Enviamos la respuesta, con un mensaje de error y un status 500 -> INTERNAL_SERVER_ERROR */
			response.put("Mensaje", "Error al eliminar la provincia de la Base de Datos");
			response.put("Error", e.getMessage().concat(": ").concat(e.getMostSpecificCause().getMessage()));
			return new ResponseEntity<Map<String, Object>>(response, HttpStatus.INTERNAL_SERVER_ERROR);
		}

		/* Enviamos la respuesta, con un mensaje de exito y un status 200 -> OK */
		response.put("Mensaje", "La provincia se elimino con exito!");
