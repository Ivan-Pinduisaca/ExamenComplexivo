...
import com.complexivo.backend.services.ServicioTipoPago;
...
public class RestTipoPago {
	...
	public ResponseEntity<List<TipoPago>> all() {
		/* Enviamos la respuesta con mensaje de exito y un status 200 -> OK */
		return ResponseEntity.ok(servicioTipoPago.findAll());
	}
	...
	public ResponseEntity<?> findId(@PathVariable Long id) {
		...
		try {
			tipoPago = servicioTipoPago.findById(id);
		...
		if (tipoPago == null) {
			...
			response.put("mensaje",
					"El tipo de pago con ID: '".concat(id.toString()).concat("' no existe en la Base de Datos"));
	...
	public ResponseEntity<?> create(@Valid @RequestBody TipoPago tipoPago, BindingResult result) {
		...
		try {
			tipoPagoNew = servicioTipoPago.save(tipoPago);
		} catch (DataAccessException e) {
			/* Enviamos la respuesta, con un mensaje de error y un status 500 -> INTERNAL_SERVER_ERROR */
			response.put("Mensaje", "Error al guardar el tipo de pago en la Base de Datos");
			response.put("Error", e.getMessage().concat(": ").concat(e.getMostSpecificCause().getMessage()));
			return new ResponseEntity<Map<String, Object>>(response, HttpStatus.INTERNAL_SERVER_ERROR);
		}

		/* Enviamos la respuesta, con un mensaje de exito y un status 201 -> CREATED */
		response.put("Mensaje", "El tipoPago se ha creado con exito!");
	...
	public ResponseEntity<?> update(@Valid @RequestBody TipoPago tipoPago, BindingResult result,
			@PathVariable Long id) {

		TipoPago tipoPagoActual = servicioTipoPago.findById(id);
		TipoPago tipoPagoUpdate = null;
		Map<String, Object> response = new HashMap<>();
		...
		if (tipoPagoActual == null) {
			...
			response.put("Mensaje", "Error, no se pudo editar. El tipo de pago con ID: "
		...
		try {
			tipoPagoActual.setTppTipoPago(tipoPago.getTppTipoPago());

			tipoPagoUpdate = servicioTipoPago.save(tipoPagoActual);
		} catch (DataAccessException e) {
			/* Enviamos la respuesta, con un mensaje de error y un status 500 -> INTERNAL_SERVER_ERROR */
			response.put("Mensaje", "Error al actualizar el tipo de pago");
			response.put("Error", e.getMessage().concat(": ").concat(e.getMostSpecificCause().getMessage()));
			return new ResponseEntity<Map<String, Object>>(response, HttpStatus.INTERNAL_SERVER_ERROR);
		}

		/* Enviamos la respuesta, con un mensaje de exito y un status 201 -> CREATED */
		response.put("Mensaje", "El tipo de pago se actualizo con exito!");
		response.put("Tipo de pago", tipoPagoUpdate);
		return new ResponseEntity<Map<String, Object>>(response, HttpStatus.CREATED);
	}
	...
	public ResponseEntity<?> delete(@PathVariable Long id) {
		...
		try {
			servicioTipoPago.deleteById(id);
		} catch (DataAccessException e) {
			/* Enviamos la respuesta, con un mensaje de error y un status 500 -> INTERNAL_SERVER_ERROR */
			response.put("Mensaje", "Error al eliminar el tipo de pago de la Base de Datos");
			response.put("Error", e.getMessage().concat(": ").concat(e.getMostSpecificCause().getMessage()));
			return new ResponseEntity<Map<String, Object>>(response, HttpStatus.INTERNAL_SERVER_ERROR);
		}

		/* Enviamos la respuesta, con un mensaje de exito y un status 200 -> OK */
		response.put("Mensaje", "El tipo de pago se elimino con exito!");
