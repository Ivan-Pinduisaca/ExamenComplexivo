...
import com.complexivo.backend.services.ServicioModulo;
...
public class RestModulo {
	...
	@GetMapping("/modulos")
	public ResponseEntity<List<Modulo>> all() {
		/* Enviamos la respuesta con mensaje de exito y un status 200 -> OK */
		return ResponseEntity.ok(servicioModulo.findAll());
	}
	...
	public ResponseEntity<?> findId(@PathVariable Long id) {
		...
		try {
			modulo = servicioModulo.findById(id);
		...
		if (modulo == null) {
			...
			response.put("mensaje",
					"El modulo con ID: '".concat(id.toString()).concat("' no existe en la Base de Datos"));
	...
	@PostMapping("/modulos")
	public ResponseEntity<?> create(@Valid @RequestBody Modulo modulo, BindingResult result) {
		...
		try {
			moduloNew = servicioModulo.save(modulo);
		} catch (DataAccessException e) {
			/* Enviamos la respuesta, con un mensaje de error y un status 500 -> INTERNAL_SERVER_ERROR */
			response.put("Mensaje", "Error al guardar el modulo en la Base de Datos");
			response.put("Error", e.getMessage().concat(": ").concat(e.getMostSpecificCause().getMessage()));
			return new ResponseEntity<Map<String, Object>>(response, HttpStatus.INTERNAL_SERVER_ERROR);
		}

		/* Enviamos la respuesta, con un mensaje de exito y un status 201 -> CREATED */
		response.put("Mensaje", "El modulo se ha creado con exito!");
	...
	public ResponseEntity<?> update(@Valid @RequestBody Modulo modulo, BindingResult result, @PathVariable Long id) {

		Modulo moduloActual = servicioModulo.findById(id);
		...
		if (moduloActual == null) {
			...
			response.put("Mensaje", "Error, no se pudo editar. El modulo con ID: "
		...
		try {
			moduloActual.setModNombre(modulo.getModNombre());
			moduloActual.setRolUsuario(modulo.getRolUsuario());

			moduloUpdate = servicioModulo.save(moduloActual);
		} catch (DataAccessException e) {
			/* Enviamos la respuesta, con un mensaje de error y un status 500 -> INTERNAL_SERVER_ERROR */
			response.put("Mensaje", "Error al actualizar el modulo");
			response.put("Error", e.getMessage().concat(": ").concat(e.getMostSpecificCause().getMessage()));
			return new ResponseEntity<Map<String, Object>>(response, HttpStatus.INTERNAL_SERVER_ERROR);
		}

		/* Enviamos la respuesta, con un mensaje de exito y un status 201 -> CREATED */
		response.put("Mensaje", "El modulo se actualizo con exito!");
	...
	public ResponseEntity<?> delete(@PathVariable Long id) {
		...
		response.put("Mensaje", "El modulo se elimino con exito!");
