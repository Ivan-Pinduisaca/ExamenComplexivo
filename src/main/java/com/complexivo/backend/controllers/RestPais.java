...
import com.complexivo.backend.services.ServicioPais;
...
public class RestPais {
	...
	public ResponseEntity<List<Pais>> all() {
		...
		return ResponseEntity.ok(servicioPais.findAll());
	}
	...
	public ResponseEntity<?> findId(@PathVariable Long id) {
		...
		if (pais == null) {
			...
			response.put("mensaje",
					"El pais con ID: '".concat(id.toString()).concat("' no existe en la Base de Datos"));
	...
	public ResponseEntity<?> update(@Valid @RequestBody Pais pais, BindingResult result, @PathVariable Long id) {
		...
		Pais paisActual = servicioPais.findById(id);
		...
		if (paisActual == null) {
			...
			response.put("Mensaje", "Error, no se pudo editar. El pais con ID: "
		...
		try {
			paisActual.setPaisNombre(pais.getPaisNombre());

			paisUpdate = servicioPais.save(paisActual);
		} catch (DataAccessException e) {
			/* Enviamos la respuesta, con un mensaje de error y un status 500 -> INTERNAL_SERVER_ERROR */
			response.put("Mensaje", "Error al actualizar el pais");
			response.put("Error", e.getMessage().concat(": ").concat(e.getMostSpecificCause().getMessage()));
			return new ResponseEntity<Map<String, Object>>(response, HttpStatus.INTERNAL_SERVER_ERROR);
		}

		/* Enviamos la respuesta, con un mensaje de exito y un status 201 -> CREATED */
		response.put("Mensaje", "El pais se actualizo con exito!");
